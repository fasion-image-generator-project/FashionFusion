...
const ImageGenerator = () => {
  ...
  const [state, setState] = useState({
    ...
    loading: false,          // 로딩 상태
    error: "",              // 에러 메시지
    selectedModel: "Stable Diffusion 1.5"  // 선택된 모델
  });
  ...
  const handleInitialGeneration = useCallback(async () => {
    if (!state.prompt || !state.selectedModel) return;
    ...
    try {
      if (USE_DUMMY_DATA) {
        ...
        setState(prev => ({
          ...prev,
          initialImage: DUMMY_IMAGES[state.selectedModel].INITIAL,
          finalImage: ""
        }));
      } else {
        const imageData = await generateInitialImage(state.prompt, state.selectedModel);
        setState(prev => ({
          ...prev,
  ...
  }, [state.prompt, state.selectedModel]);
  ...
  const handleFinalGeneration = useCallback(async () => {
    ...
    try {
      if (USE_DUMMY_DATA) {
        await new Promise(resolve => setTimeout(resolve, 1000));
        const finalImageData = DUMMY_IMAGES[state.selectedModel].FINAL;
    ...
    <ThemeProvider theme={isDarkMode ? darkTheme : lightTheme}>
      ...
      <Container>
        ...
        <MainContent>
          ...
          <ContentArea>
            <ImageControlArea>
              {/* Model Selection and Prompt Input - Only visible when no images exist */}
              {!state.initialImage && !state.finalImage && (
                <>
                  <ModelSelect>
                    <ModelOptions>
                      <div style={{ fontWeight: "bold", marginBottom: "10px", color: props => props.theme.textSecondary }}>Select Model</div>
                      <div style={{
                        display: "flex",
                        gap: "20px",
                        ...
                        {MODEL_OPTIONS.map((option) => (
                          ...
                          >
                            <input
                              ...
                              name="model"
                              value={option.id}
                              checked={state.selectedModel === option.id}
                  ...
                  >
                    ...
                    <Button
                      type="submit"
                      disabled={isGenerateDisabled || !state.selectedModel}
